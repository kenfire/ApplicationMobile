/*$Id: getSnmpAugmentTable.src,v 1.1.2.5 2012/01/09 14:24:31 sumathi.g Exp $*/
/*
...
/** 
 *  This is an example of using the SnmpAugmentTable class.
...
public class getSnmpAugmentTable implements SnmpTableListener {
    ...
    String usage = "getSnmpAugmentTable [-v version(v1,v2,v3)] [-c community] [-p port] [-t timeout] [-r retries] [-u user] [-a auth_protocol(MD5/SHA)] [-w auth_password] [-s priv_password] [-n contextname] [-i contextID] [-d debug] [ -pp priv_protocol (DES/AES-128/AES-192/AES-256/3DES) ] host tableOID MIB_files";
 ...
 if ((values[VERSION].equals("v3")) && (values[USER_NAME] == null)) opt.usage_error();
    ...
    // instantiate an SnmpAugmentTable instance
    ...
    getSnmpAugmentTable gettable = new getSnmpAugmentTable();
    table.addSnmpTableListener(gettable);
	// Set the table OID in SnmpAugmentTable.  Once specified,
    // SnmpAugmentTable will automatically get the data and send table events 
    try { 
        table.setTableObjectID( opt.remArgs[1] );
    } catch (Exception ex) {
    ...
    SnmpAugmentTable table = (SnmpAugmentTable)e.getSource();
