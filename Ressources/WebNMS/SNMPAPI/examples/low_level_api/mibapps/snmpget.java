/* $Id: snmpget.src,v 1.4.2.10 2012/01/09 14:26:34 pradheep.v Exp $ */
/*
 * @(#)snmpget.java
 ...
 * the basic SNMP operation GET using com.adventnet.snmp.snmp2 and
 * com.adventnet.snmp.mibs packages of WebNMS SNMP2 api.
 * The user could run this application by giving any one of the following usage.
 *  
 * java snmpget [options] hostname oid ...
 *
 * v1 request:
 * java snmpget [-d] [-m MIB_files] [-c community] [-p port] [-t timeout] [-r retries] host OID [OID] ...
 * e.g. 
 * java snmpget -p 161 -c public -m ../../../mibs/RFC1213-MIB adventnet 1.1.0 1.2.0
 *
 * v2c request:
 * java snmpget [-d] [-v version(v1,v2)] [-m MIB_files] [-c community] [-p port] [-t timeout] [-r retries] host OID [OID] ...
 * e.g. For v1 request give -v v1 or drop the option -v .
 * java snmpget -p 161 -v v2 -c public -m ../../../mibs/RFC1213-MIB adventnet 1.1.0 1.2.0
 * 
 * v3 request:
 * java snmpget [-d] [-v version(v1,v2,v3)] [-c community] [-m MIB_files] [-p port] [-r retries] [-t timeout] [-u user] [-a auth_protocol] [-w auth_password] [-s priv_password][-pp privProtocol(DES/AES-128/AES-192/AES-256/3DES)] [-n contextName] [-i contextID] host OID [OID] ...
 * e.g.
...
public class snmpget {
    ...
    if (values[DEBUG].equals("Set")) api.setDebug( true );
